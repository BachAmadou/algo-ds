...
// LINK TO CODEPEN VERSION: https://codepen.io/816978819/pen/oOLGZW
...
function merge(sortedArr1, sortedArr2) {
  ...
  while(pointer1 < sortedArr1.length && pointer2 < sortedArr2.length) {
    if(sortedArr1[pointer1] < sortedArr2[pointer2]) {
      result.push(sortedArr1[pointer1]);
      pointer1++;
    } else {
      result.push(sortedArr2[pointer2]);
      pointer2++;
    }
  }

  // if one of the arrays is exausted, then add the remaining elements of the standing array
  ...
  while(pointer2 < sortedArr2.length){
    result.push(soredArr2[pointer2]);
    pointer2++;
  }
